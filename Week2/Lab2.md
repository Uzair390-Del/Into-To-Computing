...
How to Check System (PC) Specifications
...
Introduction to Overleaf & LaTeX
...
The System tab shows: OS version, manufacturer, model, processor, and installed RAM.
The Display tab shows your graphics card details and driver info.
The Sound and Input tabs show audio devices and connected input hardware (mouse, keyboard, etc.)
...
Shut down and unplug the PC — avoids electrical damage or short circuits.
Open the case — use a screwdriver to remove the side panel.
Locate an available drive bay — find a free slot for the new drive.
Connect the SATA cable and power connector — SATA transfers data to the motherboard; the power cable comes from the PSU.
Secure the drive — use screws or clips so it doesn't move during use.
Close the case and power on — plug the PC back in and boot it up.
Verify installation — check that the drive appears under Storage Management in the OS.
7. Quick Recap
...
Click "Recompile" to update the preview after making changes.
...
Clone the repository (only once, to get a local copy):
git clone https://github.com/username/repository.git
Make changes to your files (edit code, add new files, etc.)
Check what changed:
git status
Stage your changes:
git add .
...
git push
Pull before you start working again (to get teammates' latest updates):
git pull
